...
public class AutoLeftScore2 extends AutoBase {
    ...
    public AutoLeftScore2(
        ...
        var autoLeftScore1AndPickCommand = new AutoLeftScore1AndPick(nt, swerveSubsystem, gripperSubsystem, jointSubsystem, extensionSubsystem, intakeMode);

        var startPosition_in = autoLeftScore1AndPickCommand.getStartPosition_in();
        var communityX_in = startPosition_in.getX() + 27.0;
        var communityY_in = startPosition_in.getY() + interNodeY_in - 1.0 + autoLeftScore1AndPickCommand.getManualOffsetY_in();
        var communityPosition_in = Vector2D.FromXY(communityX_in, communityY_in);

        var pickPouncePosition_in = autoLeftScore1AndPickCommand.getPickPouncePosition_in();

        addCommands(
            autoLeftScore1AndPickCommand,
